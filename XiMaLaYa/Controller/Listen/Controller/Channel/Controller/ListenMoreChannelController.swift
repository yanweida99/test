...
class ListenMoreChannelController: UIViewController {
    ...
    private let OneKeyListenTableViewCellID = "OneKeyListenTableViewCell"
    private let ListenChannelTableViewCellID = "ListenChannelTableViewCell"
    
    // 懒加载左边tableview
    private lazy var leftTableView: UITableView = {
        let tableView = UITableView.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth * 0.2, height: ScreenHeight))
        tableView.delegate = self
        tableView.dataSource = self
        tableView.separatorStyle = UITableViewCell.SeparatorStyle.none
        tableView.register(OneKeyListenTableViewCell.self, forCellReuseIdentifier: OneKeyListenTableViewCellID)
        // 左边cell默认选中第一个cell
    ...
    private lazy var rightTableView: UITableView = {
        let tableView = UITableView.init(frame: CGRect(x: ScreenWidth * 0.2, y: 0, width: ScreenWidth * 0.8, height: ScreenHeight), style: UITableView.Style.grouped)
        tableView.delegate = self
        tableView.dataSource = self
...
extension ListenMoreChannelController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == self.leftTableView {
            let cell: OneKeyListenTableViewCell = tableView.dequeueReusableCell(withIdentifier: OneKeyListenTableViewCellID, for: indexPath) as! OneKeyListenTableViewCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView == self.leftTableView {
            ...
            if newRow != oldRow {
                let newCell: OneKeyListenTableViewCell = tableView.cellForRow(at: indexPath) as! OneKeyListenTableViewCell
                newCell.titleLabel.textColor = ButtonColor
                newCell.lineView.isHidden = false
                
                let oldCell: OneKeyListenTableViewCell = tableView.cellForRow(at: self.lastPath!) as! OneKeyListenTableViewCell
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView == self.rightTableView {
            ...
            let oldRow = (self.lastPath != nil) ? self.lastPath?.row : -1
            if newRow != oldRow {
                let newCell: OneKeyListenTableViewCell = self.leftTableView.cellForRow(at: leftMoveToIndexPath) as! OneKeyListenTableViewCell
                newCell.titleLabel.textColor = ButtonColor
                newCell.lineView.isHidden = false
                
                let oldCell: OneKeyListenTableViewCell = self.leftTableView.cellForRow(at: self.lastPath!) as! OneKeyListenTableViewCell
                oldCell.titleLabel.textColor = UIColor.black
                oldCell.lineView.isHidden = true
            }
