...
extension AttentionViewModel {
    func refreshDataSource() {
        ...
        if let mappedObject = JSONDeserializer<EventInfosModel>.deserializeModelArrayFrom(json: json["data"]["evenInfos"].description) {
...
extension AttentionViewModel {
    ...
    func height(for commentModel: FindAContentInfo?) -> CGFloat {
        ...
        height += label.sizeThatFits(CGSize(width: ScreenWidth - 30, height: CGFloat.infinity)).height + 30
        return height
    }
}
