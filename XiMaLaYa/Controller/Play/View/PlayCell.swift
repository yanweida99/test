...
class PlayCell: UICollectionViewCell {
    ...
    private lazy var slider:UISlider = {
        ...
        slider.minimumTrackTintColor = ButtonColor
        // 滑块滑动停止后才触发ValueChanged事件
        //        slider.isContinuous = false
        
        slider.addTarget(self, action: #selector(PlayCell.change(slider:)), for: UIControl.Event.valueChanged)
        
        slider.addTarget(self, action: #selector(PlayCell.sliderDragUp(sender:)), for: UIControl.Event.touchUpInside)
        return slider
    }()
    // 当前时间
